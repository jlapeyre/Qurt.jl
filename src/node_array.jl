...
function Base.copy(ns::NodeArray{NodeT, N, IntT}) where {NodeT, N, IntT}
    return NodeArray{NodeT, N, IntT}((copy(x) for x in (ns.element, ns.wires, ns.numquwires, ns.inwiremap,
                                                        ns.outwiremap, ns.params))...)
end
...
function Base.empty!(nodes::NodeArray, nkeep=0)
    arrays = (nodes.element, nodes.wires, nodes.numquwires, nodes.inwiremap,
              nodes.outwiremap, nodes.params)
...
function Base.getindex(ns::NodeArray, i::Integer)
    return Node(ns.element[i], ns.wires[i], ns.numquwires[i],
                ns.inwiremap[i], ns.outwiremap[i], ns.params[i])
end
...
function Base.deleteat!(nodes::NodeArray, i::Integer)
    for v in (nodes.element, nodes.wires, nodes.numquwires, nodes.params,
              nodes.inwiremap, nodes.outwiremap)
        deleteat!(v, i)
    end
    return nodes
end
