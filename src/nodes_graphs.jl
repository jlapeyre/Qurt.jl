...
_find_runs(qc::Circuit, element, nwires, checkfun::F) where {F} =
    _find_runs(qc.nodes, topological_sort(qc.graph), element, nwires, checkfun)
...
function _find_runs(nodes::ANodeArrays, vertices, element::Element, ::Val{Nwires}, checkfun::F) where {Nwires, F}
    seen = Set{Int}()
    allruns = Vector{Vector{Int}}(undef, 0)
    for i in vertices
        (nodes.element[i] != element || i in seen) && continue
        length(nodes.wires[i]) == Nwires || throw(CircuitError("Excpecting $(Nwires)-wire operator named $element"))
        ...
        while true
            ...
            if flag && nodes.element[nv] == element &&
...
function find_runs_two_wires(qc::Circuit, element::Element)
    check_wires =
        verts -> (length(verts) == 2 && verts[1] == verts[2]) ? (verts[1], true) : (0, false)
    _find_runs(qc, element, Val(2), check_wires)
end
